...
export default function Login({ onSwitch, onSuccess }) {
  ...
  const submit = async (ev) => {
    ...
    try {
      ...
      if (!res.ok) {
        if (data.locked && data.retry_after_minutes) {
          setLockedUntil(Date.now() + data.retry_after_seconds * 1000);
          setAlert({
            type: "error",
            msg: `Too many failed attempts. Try again after ${data.retry_after_minutes} minute(s).`,
          });
        } else {
